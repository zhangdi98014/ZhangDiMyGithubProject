    ...
    public boolean preHandle(HttpServletRequest request,
        ...
        if (login == null) {
            ...
            if (requestUri.contains("/admin/")) {
                response.sendRedirect(contextPath + "/commonapi/at_login"); //强制跳转到登录页面

                return false;
            }

            if (requestUri.contains("/user/")) {
                response.sendRedirect(contextPath + "/commonapi/user_login"); //强制跳转到登录页面

                return false;
            }

            if (requestUri.contains("/tz/")) {
                response.sendRedirect(contextPath + "/commonapi/at_login"); //强制跳转到登录页面
            ...
            if (login.getLoginType() == 2) {
                if ((requestUri.contains("tz") == false) &&
                        (requestUri.contains("commonapi") == false)) { //当前登录角色为团长,但请求其他角色使用的接口,即盗链,这种需要拦截并提示错误
                    response.sendRedirect(contextPath +
                        "/commonapi/error?msg=-2");
