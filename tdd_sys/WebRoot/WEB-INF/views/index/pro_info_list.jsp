...
<div class="left-nav">
<a href="javascript:;" data-id="" ><div class="title">所有分类</div></a>
<div class="list-box">
<dl>
<c:if test="${type==null}">
<dd onclick="typeClick(this)"  data-id=""><a href="javascript:;" ><font style="color:red">所有分类</font></a></dd>
</c:if>
<c:if test="${type!=null}">
<dd onclick="typeClick(this)"  data-id=""><a href="javascript:;" ><font>所有分类</font></a></dd>
</c:if>
<c:forEach var="item2"   items="${pl}">
<c:if test="${type==item2.id }">
<dd  onclick="typeClick(this)"  data-id="${item2.id}"><a href="javascript:;"><font style="color:red">${item2.name}</font></a></dd>
</c:if>
<c:if test="${type!=item2.id }">
<dd onclick="typeClick(this)"  data-id="${item2.id}"><a href="javascript:;" ><font>${item2.name }</font></a></dd>
</c:if>
</c:forEach>
</dl>
</div>
...
$(function(){
	...
	function typeClick(e) {
		var type = $(e).attr("data-id");
		window.location.href ='${pageContext.request.contextPath}/commonapi/index/query_list?type='+type+'';
		}
	function ajaxList(page) {
		...
		$.ajax({
			type : 'get',
			url : "${pageContext.request.contextPath}/commonapi/index/search",
			data : {
				"page":page,
  				...
  				success : function(result) {
  					...
  					for (var i = 0; i < rows.length; i++) {
  						html+='<div class="item">'
  						+'<div class="img">'
  						+'<a href="${pageContext.request.contextPath}/commonapi/index/detail?id='+rows[i].id+'"><img src="'+rows[i].img+'" style="width:280px;height:230px;"></a>'
  		                +'</div>';
  		                html+='<div class="text">';
  		                html+='<p class="title">'+rows[i].title+'</p>';
  		                html+='<p class="price">';
  		                html+='<span class="pri">￥'+rows[i].price+'元</span>';
  		                html+='<span class="nub">'+rows[i].type+'</span>';
  		                html+='</p>';
  		                html+='<p class="price"><span class="nub">销量：'+rows[i].saleNum+'</span></p>';
  		                html+='</div>';
   		              	html+='</div>';
   		              	}
  					$("#list-cont").html(html);	
