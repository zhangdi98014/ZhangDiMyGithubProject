...
<a href="${pageContext.request.contextPath}/tz/tg_info/add" class="btn btn-success radius">发起团购</a>
</span>
...
<script type="text/javascript" src="${pageContext.request.contextPath}/static/common/utils/listutils.js?v=7209"></script>
...
$(function(){
	...
	function ajaxList(page) {
		...
		$.ajax({
			type : 'get',
			url : "${pageContext.request.contextPath}/tz/tg_info/queryList",
			data : {
				"page":page,
				"tgTitle":tgTitle,
				"tgStatus":tgStatus
				...
				success : function(result) {
					...
					for (var i = 0; i < rows.length; i++) {
						...
						html+='<td>';
						if(rows[i].tgInfo.tgStatus=='1'){
							html+='<a href="${pageContext.request.contextPath}/tz/tg_info/update?id='+rows[i].tgInfo.id+'" class="btn btn-primary  radius btnStyle">修改</a>';
							}
						if(rows[i].tgInfo.tgStatus=='1'){
							html+='<a href="javascript:void(0)" onclick="del(this)" data-id="'+rows[i].tgInfo.id+'" class="btn btn-danger  radius btnStyle">删除</a>';
							}
						if(rows[i].tgInfo.tgStatus=='5'){
							html+='<a href="javascript:void(0)" onclick="updateByDefault(this)" data-id="'+rows[i].tgInfo.id+'" act-type="1" class="btn btn-warning radius btnStyle">结束活动</a>';
							}
						html+='</td>';
						html+='</tr>';
						}
	...
	function updateByDefault(e){
		...
		if(r==true){
			...
			$.ajax({
				type : 'get',
				url : "${pageContext.request.contextPath}/tz/tg_info/updateByDefault",
				data : {
					"id":id
					},
					success : function(result) {
	...
	function del(e){
		if(window.confirm('你确定要删除该团购活动吗')){
			var id = $(e).attr("data-id");
			$.ajax({
				type : 'get',
				url : "${pageContext.request.contextPath}/tz/tg_info/del",
				data : {
					"id":id
					},
					success : function(result) {
