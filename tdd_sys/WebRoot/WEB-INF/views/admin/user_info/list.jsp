...
</i>用户<a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont"></i></a></nav>
...
<div style="display:inline;margin-top:30px"><span style="width:200px;margin-left:50px;">收货人姓名</span>
<input type="text" class="input-text"  style="width:250px;display:inline"  placeholder="请输入收货人姓名进行查询" id="contactName">
</div>
...
<script type="text/javascript" src="${pageContext.request.contextPath}/static/common/utils/listutils.js?v=3870"></script>
		...
		data : {
			...
			success : function(result) {
				...
				for (var i = 0; i < rows.length; i++) {
					...
					var contactNameVal=setNullToEmpty(rows[i].userInfo.contactName);
					html+='<td>'+ contactNameVal+'</td>';
					var contactCelPhoneVal=setNullToEmpty(rows[i].userInfo.contactCelPhone);
					html+='<td>'+ contactCelPhoneVal+'</td>';
					var contactCityVal=setNullToEmpty(rows[i].userInfo.contactCity);
					html+='<td>'+ contactCityVal+'</td>';
					var detailAddressVal=setNullToEmpty(rows[i].userInfo.detailAddress);
