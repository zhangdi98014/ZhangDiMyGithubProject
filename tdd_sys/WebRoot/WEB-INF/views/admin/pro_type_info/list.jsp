...
<nav class="breadcrumb"><i class="Hui-iconfont"></i>商品类型<a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont"></i></a></nav>
<div class="page-container">
<div style="display:inline;margin-top:30px"><span style="width:200px;margin-left:50px;">类型名</span>
<input type="text" class="input-text"  style="width:250px;display:inline"  placeholder="请输入类型名进行查询" id="name">
</div>
...
<a href="${pageContext.request.contextPath}/admin/pro_type_info/add" class="btn btn-success radius">新增</a>
</span>
...
<tbody id="pro_type_info_body">
</tbody>
</table>
<div id="pro_type_info_bar" class="pagination" style="margin-top:20px;">
</div>
<div>
</div>
</div>
...
<script type="text/javascript" src="${pageContext.request.contextPath}/static/common/utils/listutils.js?v=8581"></script>
...
function ajaxList(page) {
	...
	$.ajax({
		type : 'get',
		url : "${pageContext.request.contextPath}/admin/pro_type_info/queryList",
		data : {
			"page":page,
			"name":name
			},
			success : function(result) {
				...
				for (var i = 0; i < rows.length; i++) {
					html += '<tr>' ;
					var nameVal=setNullToEmpty(rows[i].proTypeInfo.name);
					html+='<td>'+ nameVal+'</td>';
					html+='<td>';
					html+='<a href="${pageContext.request.contextPath}/admin/pro_type_info/update?id='+rows[i].proTypeInfo.id+'" class="btn btn-primary  radius btnStyle">修改</a>';
					html+='<a href="javascript:void(0)" onclick="del(this)" data-id="'+rows[i].proTypeInfo.id+'" class="btn btn-danger  radius btnStyle">删除</a>';
					html+='</td>';
					html+='</tr>';
					}
				if (rows.length == 0) {
				...
				$("#pro_type_info_body").html(html);
				new myPagination({
					id: 'pro_type_info_bar',
					curPage:page, //初始页码
	...
	function del(e){
		if(window.confirm('你确定要删除该商品类型吗')){
			var id = $(e).attr("data-id");
			$.ajax({
				type : 'get',
				url : "${pageContext.request.contextPath}/admin/pro_type_info/del",
				data : {
					"id":id
					},
					success : function(result) {
