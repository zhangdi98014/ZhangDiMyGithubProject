...
<nav class="breadcrumb"><i class="Hui-iconfont"></i>商品<a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont"></i></a></nav>
<div class="page-container">
<div style="display:inline;margin-top:30px"><span style="width:200px;margin-left:50px;">商品标题</span>
<input type="text" class="input-text"  style="width:250px;display:inline"  placeholder="请输入商品标题进行查询" id="title">
</div>
<div style="display:inline;margin-top:30px"><span style="width:200px;margin-left:50px;">商品类型</span>
<select  id="proTypeId" style="height:30px;width:250px;display:inline" >
...
<a href="${pageContext.request.contextPath}/admin/pro_info/add" class="btn btn-success radius">新增</a>
</span>
...
<div id="pro_info_bar" class="pagination" style="margin-top:20px;">
</div>
<div>
</div>
</div>
...
<script type="text/javascript" src="${pageContext.request.contextPath}/static/common/utils/listutils.js?v=5256"></script>
...
$(function(){
	...
	function ajaxList(page) {
		...
		$.ajax({
			type : 'get',
			url : "${pageContext.request.contextPath}/admin/pro_info/queryList",
			data : {
				"page":page,
				"title":title,
				"proTypeId":proTypeId
				...
				success : function(result) {
					...
					for (var i = 0; i < rows.length; i++) {
						html += '<tr>' ;
						var proImgStr = rows[i].proInfo.proImg;
						if(proImgStr!=null){
							var proImgSplit = proImgStr.split(";");
							var tmp = '';
							for(var j=0;j<proImgSplit.length;j++){
								if(proImgSplit[j]!=''){
									tmp+='<div ><img src="'+proImgSplit[j]+'" style="width:100px" onclick="maxImg(this)"></div>';
									}
								}
							html+='<td>'+tmp+'</td>';
							}else{
						...
						var titleVal=setNullToEmpty(rows[i].proInfo.title);
						html+='<td>'+ titleVal+'</td>';
						var proTypeIdVal=setNullToEmpty(rows[i].proTypeIdStr);
						html+='<td>'+ proTypeIdVal+'</td>';
						var proPriceVal=setNullToEmpty(rows[i].proInfo.proPrice);
						html+='<td>'+ proPriceVal+'</td>';
						var sctockNumVal=setNullToEmpty(rows[i].proInfo.sctockNum);
						html+='<td>'+ sctockNumVal+'</td>';
						var saleNumVal=setNullToEmpty(rows[i].proInfo.saleNum);
						html+='<td>'+ saleNumVal+'</td>';
						html+='<td>';
						html+='<a href="${pageContext.request.contextPath}/admin/pro_info/detail1?id='+rows[i].proInfo.id+'" class="btn btn-success radius btnStyle">查看详情</a>';
						html+='<a href="${pageContext.request.contextPath}/admin/pro_info/update?id='+rows[i].proInfo.id+'" class="btn btn-primary  radius btnStyle">修改</a>';
						html+='<a href="javascript:void(0)" onclick="del(this)" data-id="'+rows[i].proInfo.id+'" class="btn btn-danger  radius btnStyle">删除</a>';
						html+='</td>';
						html+='</tr>';
						}
					if (rows.length == 0) {
	...
	function del(e){
		if(window.confirm('你确定要删除该商品吗')){
			...
			$.ajax({
				type : 'get',
				url : "${pageContext.request.contextPath}/admin/pro_info/del",
				data : {
					"id":id
					},
					success : function(result) {
