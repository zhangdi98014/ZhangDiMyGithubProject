...
<nav class="breadcrumb"><i class="Hui-iconfont"></i>团长<a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont"></i></a></nav>
...
<a href="${pageContext.request.contextPath}/admin/tz_info/add" class="btn btn-success radius">新增</a>
</span>
...
<tbody id="tz_info_body">
</tbody>
</table>
<div id="tz_info_bar" class="pagination" style="margin-top:20px;">
</div>
<div>
</div>
</div>
...
<script type="text/javascript" src="${pageContext.request.contextPath}/static/common/utils/listutils.js?v=2225"></script>
...
function ajaxList(page) {
	...
	$.ajax({
		type : 'get',
		url : "${pageContext.request.contextPath}/admin/tz_info/queryList",
		data : {
			"page":page,
			"name":name
			},
		success : function(result) {
			...
			for (var i = 0; i < rows.length; i++) {
				html += '<tr>' ;
				var nameVal=setNullToEmpty(rows[i].tzInfo.name);
				html+='<td>'+ nameVal+'</td>';
				var passWordVal=setNullToEmpty(rows[i].tzInfo.passWord);
				html+='<td>'+ passWordVal+'</td>';
				html+='<td>';
				html+='<a href="${pageContext.request.contextPath}/admin/tz_info/update?id='+rows[i].tzInfo.id+'" class="btn btn-primary  radius btnStyle">修改</a>';
				html+='<a href="javascript:void(0)" onclick="del(this)" data-id="'+rows[i].tzInfo.id+'" class="btn btn-danger  radius btnStyle">删除</a>';
				html+='</td>';
				html+='</tr>';
				}
			if (rows.length == 0) {
	...
	function del(e){
		if(window.confirm('你确定要删除该团长吗')){
			var id = $(e).attr("data-id");
			$.ajax({
				type : 'get',
				url : "${pageContext.request.contextPath}/admin/tz_info/del",
				data : {
					"id":id
					},
				success : function(result) {
