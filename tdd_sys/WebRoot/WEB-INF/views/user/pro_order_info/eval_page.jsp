...
<div class="row cl" id="evalStarParentContent" style="margin-top:20px;margin-left:20px;" >
<label class="form-label col-xs-4 col-sm-2">
 评价星级
</label>
<div class="formControls col-xs-8 col-sm-9">
<div class="evalStar" id="evalStar" init-val="<c:if test="${data.evalStar==null}">0</c:if><c:if test="${data.evalStar!=null}">${data.evalStar}</c:if>"> </div>
</div>
</div>
<div class="row cl" id="evalDetailParentContent" style="margin-top:20px;margin-left:20px;" >
<label class="form-label col-xs-4 col-sm-2">
评价详情
</label>
<div class="formControls col-xs-8 col-sm-9">
<input type="text" class="input-text"  style="width:500px;display:inline" value="${data.evalDetail}" placeholder="请输入评价详情" id="evalDetail">
</div>
</div>
</div>
<hr>
...
<script type="text/javascript" src="${pageContext.request.contextPath}/static/common/utils/listutils.js?v=8571"></script>
<script type="text/javascript">
$(function(){
	initStarEval('evalStar','${pageContext.request.contextPath}/static/eval');
	})
function submitData(){
	var evalStar= getStarByDomId('evalStar');
	var evalDetail= $('#evalDetail').val();
	if(evalStar!=''){
		if(isIntNum(evalStar)==false){
			alert("评价星级必须填入整数");
			return;
			}
		}
	$.ajax({
		type: 'post',
		url: '${pageContext.request.contextPath}/user/pro_order_info/eval_submit',
		data:{
			"id":'${data.id}',
			"evalStar":evalStar,
