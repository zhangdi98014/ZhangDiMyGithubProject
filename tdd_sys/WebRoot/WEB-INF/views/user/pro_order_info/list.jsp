...
<span style="width:200px;margin-left:50px;">联系人电话</span>
...
<span class="l" id="globalBtns"></span>
<span id="batchUpdateDiv"></span>
</div>
...
<script type="text/javascript" src="${pageContext.request.contextPath}/static/common/utils/listutils.js?v=9322"></script>
...
function ajaxList(page) {
	...
	$.ajax({
		type : 'get',
		url : "${pageContext.request.contextPath}/user/pro_order_info/queryList",
		data : {
			"page":page,
			"orderNo":orderNo,
			"proName":proName,
		...
		for (var i = 0; i < rows.length; i++) {
			...
			html+='<td>';
			if(rows[i].proOrderInfo.orderStatus=='3'){
				html+='<a href="javascript:void(0)" onclick="updateByDefault(this)" data-id="'+rows[i].proOrderInfo.id+'" act-type="1" class="btn btn-warning radius btnStyle">确认收货</a>';
				}
			if(rows[i].proOrderInfo.orderStatus=='4'){
				html+='<a href="${pageContext.request.contextPath}/user/pro_order_info/eval?id='+rows[i].proOrderInfo.id+'" class="btn btn-primary  radius btnStyle">评价</a>';
				}
			html+='</td>';
			html+='</tr>';
			}
		...
		function updateByDefault(e){
			...
			if(r==true){
				...
				$.ajax({
					type : 'get',
					url : "${pageContext.request.contextPath}/user/pro_order_info/updateByDefault",
					data : {
						"id":id
						},
						success : function(result) {
