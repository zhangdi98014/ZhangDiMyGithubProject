...
<script type="text/javascript" src="${pageContext.request.contextPath}/static/common/utils/listutils.js?v=9073"></script>
...
$(function(){
	...
	function ajaxList(page) {
		...
		$.ajax({
			type : 'get',
			url : "${pageContext.request.contextPath}/user/tg_order_info/queryList",
			data : {
				"page":page,
				"orderNo":orderNo,
				"tgTitle":tgTitle,
				...
				success : function(result) {
					...
					for (var i = 0; i < rows.length; i++) {
						...
						html+='<td>';
						if(rows[i].tgOrderInfo.tgOrderStatus=='3'){
							html+='<a href="javascript:void(0)" onclick="updateByDefault(this)" data-id="'+rows[i].tgOrderInfo.id+'" act-type="1" class="btn btn-warning radius btnStyle">确认收货</a>';
							}
						html+='</td>';
						html+='</tr>';
						}
	...
	function updateByDefault(e){
		...
		if(r==true){
			...
			$.ajax({
				type : 'get',
				url : "${pageContext.request.contextPath}/user/tg_order_info/updateByDefault",
				data : {
					"id":id
					},
					success : function(result) {
